...
public class KuwaharaRenderFeature : ScriptableRendererFeature {
    //public static LayerMask exclusionLayer;
    private const int FilterKernelSize = 32;
    
    public Settings settings;

    private DepthOnlyPass depthOnlyPass;
    private KuwaharaFeaturePass renderPass;
    ...
    {
        var structureTensorMaterial = CoreUtils.CreateEngineMaterial("Hidden/Kuwahara/Structure Tensor");
        var kuwaharaFilterMaterial = CoreUtils.CreateEngineMaterial("Hidden/Kuwahara/Kuwahara Shader");
        var lineIntegralConvolutionMaterial = CoreUtils.CreateEngineMaterial("Hidden/Kuwahara/Line Integral Convolution");
        var compositorMaterial = CoreUtils.CreateEngineMaterial("Hidden/Kuwahara/Compositor");

        renderPass = new KuwaharaFeaturePass(structureTensorMaterial, kuwaharaFilterMaterial, lineIntegralConvolutionMaterial, compositorMaterial);
        ...
        FilterKernelSize,
        settings.kuwaharaFilterSettings.filterKernelSectors,
        settings.kuwaharaFilterSettings.filterKernelSmoothness);
        settings.kuwaharaFilterSettings.filterKernelTexture = texture;
        
        //depthOnlyPass = new DepthOnlyPass(RenderPassEvent.BeforeRenderingPostProcessing, RenderQueueRange.all, exclusionLayer);
        //depthTexture.("_CameraDepthTexture"); fix render handle .init
    }
    ...
    {
        public KuwaharaFilterSettings kuwaharaFilterSettings;
