...
public class DamageIndicatorManager : MonoBehaviour {
    ...
    private void SpawnDamageText(BattleStateMachine.BattleState battleState, BattleStateInput input) {
        if (battleState is BattleStateMachine.AnimateState) {
            ...
            int damage = input.SkillPrep.skill.ComputeSkillActionValues(target, input.CurrTurn()).immediateDamage;
            if (damage > 0) {
                ...
                RunTextAnimation(text.transform);
            }
        }
    }

    private void RunTextAnimation(Transform text) {
        if (_textQueue == null) {
            _textQueue = TextAnimationAction(text);
            StartCoroutine(_textQueue);
        }
    }

    private IEnumerator TextAnimationAction(Transform text) {
